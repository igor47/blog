...
I looked around for a "low end box":lowendbox, since I didn't need much power. While not being very server-wise, I still needed some. I had at this point already written "What I wish a Ruby programmer had told me one year ago":what_i_wish which became quite popular, with something like 20.000 hits the first day. A social media attack like that, requires some server power. Luckily, the server I had my blog hosted on at that point, could easily manage such a load. However, when I wanted to move my blog to a less powerful server, my own, I'd still need some power in order to handle these attacks. I wanted to make the move because I didn't want to be limited to Wordpress, or PHP-based stuff. I was set to develop my own little solution, for the fun and learning process of it.
...
bq. For a personal Linux server, it doesn't matter too much which distro. you are using. As long as you feel confident with it.[1]
